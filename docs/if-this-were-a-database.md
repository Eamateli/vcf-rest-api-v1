...
would be a second copy that disagrees with the file the moment either is written.

This page answers the follow-up: *what would the schema be, and what would change?* The answer
explains several choices in the code, and the file-based limitations documented in the README
have exact database equivalents.
